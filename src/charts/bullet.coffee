  ...
  d3.bullet = ->
    ...
    bullet = (g) ->
      g.each (d, i) ->
        ...
        range.transition().duration(duration).attr('x', if reverse then x1 else 0).attr('width', w1).attr 'height', height
        # Update the measure rects.
        measure = g.selectAll('rect.measure').data(measurez)
        measure.enter().append('rect').attr('class', (d, i) ->
          'measure s' + i
        ).attr('width', w0).attr('height', height / 3).attr('x', if reverse then x0 else 0).attr('y', height / 3).transition().duration(duration).attr('width', w1).attr 'x', if reverse then x1 else 0
        measure.transition().duration(duration).attr('width', w1).attr('height', height / 3).attr('x', if reverse then x1 else 0).attr 'y', height / 3
